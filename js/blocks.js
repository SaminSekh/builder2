...
/**
 * Resolves dynamic tokens like {year}, {month}, {date}, {time}, {day} in text.
 * These update in real-time on the live site.
 */
function resolveTokens(str) {
  ...
  return str
    ...
    .replace(/\{full-date\}/gi, String(now.getDate()).padStart(2,'0') + '/' + String(now.getMonth()+1).padStart(2,'0') + '/' + now.getFullYear())
    .replace(/\{iso-date\}/gi, now.toISOString().split('T')[0])
    .replace(/\{timezone\}/gi, Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC');
}

/**
 * Escapes HTML special characters to prevent XSS injection.
 * Also resolves dynamic tokens like {year}, {date}, etc.
 * Use this for ALL user-provided text interpolated into HTML strings.
 */
function escHtml(str) {
  return String(resolveTokens(str == null ? '' : String(str)))
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
...
/**
 * Escapes a value for use inside an HTML attribute (href, src, style, etc.)
...
/**
 * Safe segment derivation — returns the segments array without mutating props.
 * If props.segments exists, returns it. Otherwise returns the fallback.
 * NEVER assigns back to props.
 */
function deriveSegments(props, fallback) {
    return (props.segments && props.segments.length > 0) ? props.segments : fallback;
}

/**
 * Safe item-segment derivation — returns segments for a repeater item
 * without mutating the item object. Syncs current item field values
 * into the segment values for rendering.
 */
function deriveItemSegments(item, segmentDefs) {
    if (item.segments && item.segments.length > 0) {
        // Return a copy with synced values from the item's top-level fields
        return item.segments.map((seg, i) => {
            const def = segmentDefs[i];
            if (!def) return seg;
            // Sync the current field value into the segment for rendering
            const value = item[def.field] !== undefined ? item[def.field] : seg.value;
            return { ...seg, value };
        });
    }
    // No segments yet — build from definitions
    return segmentDefs.map(def => ({
        type: def.type,
        value: item[def.field] !== undefined ? item[def.field] : (def.defaultValue || ''),
        label: def.label || undefined
    }));
}

/**
 * Creates a render-safe props proxy that prevents mutation.
 * In production, this is a no-op passthrough for performance.
 * In debug mode, it logs warnings when render functions try to mutate props.
 */
function safeRenderProps(props) {
    if (!window.SOCOX_DEBUG) return props;
    return new Proxy(props, {
        set(target, key, value) {
            console.warn(`[BlockRender] Attempted to mutate props.${String(key)} during render. This is not allowed.`);
            return true; // Silently prevent the mutation
        }
    });
}

/**
 * Resolves the display text for a sub-element path.
 * If subStyles[path].text exists, returns that override.
 * Otherwise returns the base value from props.
 * This ensures render output always reflects the latest edited state.
 */
function resolveSubText(props, path, baseValue) {
    if (!props || !props.subStyles) return resolveTokens(baseValue);
    const override = props.subStyles[path];
    if (override && override.text !== undefined) return resolveTokens(override.text);
    return resolveTokens(baseValue);
}

/**
 * Generates a <style> tag for background-image-only blur using ::before pseudo-element.
 * Call this in any block render when props.bgBlur > 0 and props.bgImage is set.
 * The parent element must have position:relative and overflow:hidden.
 */
function generateBgBlurStyle(uid, props) {
    if (!props.bgImage || !props.bgBlur || parseInt(props.bgBlur) <= 0) return '';
    const overlayOpacity = props.bgOverlayOpacity !== undefined ? props.bgOverlayOpacity : 0.4;
    const overlayColor = props.bgOverlayColor || '0,0,0';
    const bgSize = props.bgSize || 'cover';
    const bgPosition = props.bgPosition || 'center';
    const blur = parseInt(props.bgBlur);
...
#${uid}::before {
    ...
    position:absolute;
    inset:-${blur * 2}px;
    background: linear-gradient(rgba(${overlayColor},${overlayOpacity}), rgba(${overlayColor},${overlayOpacity})), url(${props.bgImage}) ${bgPosition}/${bgSize} no-repeat;
    filter: blur(${blur}px);
...
const BlockTypes = {
  ...
  navbar: {
    ...
    render(props) {
      ...
      const btn = props.showButton ? `<a href="${escAttr(props.buttonHref || '#')}" class="nav-btn" style="background:${props.buttonColor};color:#fff;padding:8px 20px;border-radius:6px;text-decoration:none;font-weight:600;font-size:0.85rem;transition:opacity .2s;" onmouseover="this.style.opacity='.85'" onmouseout="this.style.opacity='1'">${escHtml(props.buttonText)}</a>` : '';
      ...
      const styledBtn = props.showButton ? `<a href="${escAttr(props.buttonHref || '#')}" class="nav-btn" style="${buttonStyle}" onmouseover="${navStyle === 'app' ? "this.style.transform='translateY(-1px)';this.style.opacity='.92'" : "this.style.opacity='.85'"}" onmouseout="${navStyle === 'app' ? "this.style.transform='translateY(0)';this.style.opacity='1'" : "this.style.opacity='1'"}">${escHtml(props.buttonText)}</a>` : '';
...
`
  ...
  <div style="${chrome.brand}">
    ${brandBadge}${logo}<span style="${chrome.brandText}">${escHtml(props.brand)}</span>
  </div>
  ...
  <div class="nav-links-container" style="${chrome.linksWrap}">
    <div class="nav-links" style="${chrome.links}">
      ${(props.links || []).map((link, i) => {
        const path = `links.${i}`;
        const base = linkBase;
        // Parse dropdown children from childrenText or children array
        ...
        const dropdownHtml = hasDropdown ? `<div class="sf-nav-dropdown" style="position:absolute;top:100%;left:0;min-width:180px;background:${props.bgColor || '#0f172a'};border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:6px 0;opacity:0;visibility:hidden;transform:translateY(6px);transition:all 0.25s;z-index:1000;box-shadow:0 10px 30px rgba(0,0,0,0.4);">
            ${children.map(c => `<a href="${escAttr(c.href || '#')}" style="display:block;padding:9px 18px;color:${props.textColor};text-decoration:none;font-size:0.82rem;transition:background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.08)'" onmouseout="this.style.background='none'">${escHtml(c.label)}</a>`).join('')}
        </div>` : '';
        return `<div class="sf-nav-link-wrap" style="position:relative;" data-sf-path="${path}" onmouseover="var d=this.querySelector('.sf-nav-dropdown');if(d){d.style.opacity='1';d.style.visibility='visible';d.style.transform='translateY(0)';}" onmouseout="var d=this.querySelector('.sf-nav-dropdown');if(d){d.style.opacity='0';d.style.visibility='hidden';d.style.transform='translateY(6px)';}">
            <a href="${escAttr(link.href || '#')}" class="nav-link" style="${BlockTypes.applySubStyle(props, path, base)}display:flex;align-items:center;gap:4px;"
               onmouseover="this.style.opacity='.7'" onmouseout="this.style.opacity='1'">${escHtml(link.label)}${hasDropdown ? ' <i class="fa-solid fa-chevron-down" style="font-size:0.55rem;opacity:0.6;"></i>' : ''}</a>
            ${dropdownHtml}
        </div>`;
      }).join('')}
  ...
  <script>
    if (window.Cart) {
        Cart.setConfig({
            whatsapp: '${escAttr(props.whatsapp || '')}',
            telegram: '${escAttr(props.telegram || '')}'
        });
    }
...
/* Dropdown hover */
#${uid} .sf-nav-link-wrap:hover .sf-nav-dropdown { opacity:1; visibility:visible; transform:translateY(0); }
...
#canvasFrame.tablet #${uid} .apk-hide-desktop { display: none !important; }
  ...
  hero: {
    ...
    render(props) {
      ...
      // Background is now handled by applyLayout (supports bgImage for ALL blocks)
      // Only set bgColor here as fallback when no bgImage
      const bgColorStyle = !props.bgImage ? `background: ${props.bgColor};` : '';
      const layout = BlockTypes.applyLayout(props);

      // Derive CTAs without mutating props
      // Always prefer ctaText/cta2Text as the source of truth (editable from panel)
      let ctas;
      if (props.ctas && props.ctas.length > 0) {
          // Sync text from legacy props into existing ctas array (panel edits ctaText/cta2Text)
          ctas = props.ctas.map((c, i) => {
              const copy = { ...c };
              if (i === 0 && props.ctaText !== undefined) copy.text = props.ctaText;
              if (i === 1 && props.cta2Text !== undefined) copy.text = props.cta2Text;
              return copy;
          });
      } else {
          ctas = [];
          if (props.ctaText) ctas.push({ text: props.ctaText, href: props.ctaHref, primary: true });
          if (props.cta2Text) ctas.push({ text: props.cta2Text, href: props.cta2Href, primary: false });
      }

      // Derive segments without mutating props
      const segments = props.segments && props.segments.length > 0 ? props.segments : [
          { type: 'title' },
          { type: 'subtitle' },
          { type: 'ctas' }
      ];

      const ctaHtml = ctas.map((c, i) => {
          ...
          // Derive segments without mutating the CTA object
          const ctaSegments = c.segments && c.segments.length > 0
            ? c.segments
            : [{ type: 'text', value: c.text || 'Button' }];

          const nestedHtml = ctaSegments.map((seg, si) => {
              const path = `ctas.${i}.segments.${si}`;
              const val = si === 0 ? (c.text || seg.value || 'Button') : seg.value;
              if (seg.type === 'text') return `<span data-sf-path="${path}" style="${BlockTypes.applySubStyle(props, path)}">${escHtml(val)}</span>`;
              if (seg.type === 'icon') return `<i data-sf-path="${path}" class="fa-solid ${escAttr(val)}" style="${BlockTypes.applySubStyle(props, path, 'margin-right:8px;')}"></i>`;
              return '';
          }).join('');

          return `<a data-sf-path="ctas.${i}" href="${escAttr(c.href || '#')}" class="${isP ? 'hero-btn' : 'hero-btn-sec'}" style="${style}">${nestedHtml}</a>`;
      }).join('');

      const segsHtml = segments.map((s, i) => {
          const path = `segments.${i}`;
          if (s.type === 'title') {
              const titleText = resolveSubText(props, path, props.title);
              ...
              return `<h1 data-sf-path="${path}" style="${BlockTypes.applySubStyle(props, path, base)}" data-initial-value="${escAttr(titleText)}">${escHtml(titleText)}</h1>`;
          }
          if (s.type === 'subtitle') {
              const subtitleText = resolveSubText(props, path, props.subtitle);
              ...
              return `<p data-sf-path="${path}" style="${BlockTypes.applySubStyle(props, path, base)}" data-initial-value="${escAttr(subtitleText)}">${escHtml(subtitleText)}</p>`;
          }
          if (s.type === 'ctas') {
      ...
      return `${generateBgBlurStyle(uid, props)}<section id="${escAttr(uid)}" class="sf-hero ${escAttr(props.customClass || '')}" style="${bgColorStyle} color: ${props.textColor}; ${layout} align-items:center; justify-content:center;">
  ...
  about: {
    ...
    render(props) {
      ...
      const features = (props.features || []).map((f, i) => {
        // Derive segments without mutating the feature object
        const fSegments = f.segments && f.segments.length >= 2
            ? f.segments
            : [
        ...
        const nestedHtml = fSegments.map((seg, si) => {
            const iconVal = si === 0 ? (f.icon || seg.value || 'fa-solid fa-check') : seg.value;
            const textVal = si === 1 ? (f.text || seg.value || 'Feature') : seg.value;
            if (seg.type === 'icon') return `<i data-sf-path="features.${i}.segments.${si}" class="${escAttr(iconVal)}" style="color:${props.accentColor}; margin-right:12px;"></i>`;
            if (seg.type === 'text') return `<span data-sf-path="features.${i}.segments.${si}" style="color:inherit;" data-initial-value="${escAttr(textVal)}">${escHtml(textVal)}</span>`;
            return '';
        }).join('');
      ...
      const segsHtml = segments.map((s, i) => {
          const path = `segments.${i}`;
          if (s.type === 'badge') {
              const badgeText = resolveSubText(props, path, props.badge);
              ...
              return `<div data-sf-path="${path}" style="text-align:${props.badgeAlign || 'left'}; margin-bottom:16px;">
                <span style="${BlockTypes.applySubStyle(props, path, base)}">${escHtml(badgeText)}</span>
              </div>`;
          }
          if (s.type === 'title') {
              const titleText = resolveSubText(props, path, props.title);
              const base = `font-size:${props.titleSize}; color:inherit; margin-bottom:24px; line-height:1.2; text-align:${props.titleAlign || 'left'};`;
              return `<h2 data-sf-path="${path}" style="${BlockTypes.applySubStyle(props, path, base)}" data-initial-value="${escAttr(titleText)}">${escHtml(titleText)}</h2>`;
          }
          if (s.type === 'text') {
              const textContent = resolveSubText(props, path, props.text);
              ...
              return `<p data-sf-path="${path}" style="${BlockTypes.applySubStyle(props, path, base)}" data-initial-value="${escAttr(textContent)}">${escHtml(textContent)}</p>`;
          }
          if (s.type === 'features') {
  ...
  services: {
    ...
    render(props) {
      ...
      const cards = (props.items || []).map((item, i) => {
        // Derive segments without mutating the item object
        const itemSegments = item.segments && item.segments.length >= 3
            ? item.segments
            : [
        ...
        const nestedHtml = itemSegments.map((seg, si) => {
            const path = `items.${i}.segments.${si}`;
            const val = si === 0 ? (item.icon || seg.value) : si === 1 ? (item.title || seg.value) : (item.desc || seg.value);
            if (seg.type === 'icon') return `<div data-sf-path="${path}" style="${BlockTypes.applySubStyle(props, path, 'font-size:2.5rem;margin-bottom:14px;')}">${escHtml(val)}</div>`;
            if (seg.type === 'title') return `<h3 data-sf-path="${path}" style="${BlockTypes.applySubStyle(props, path, 'color:inherit;font-weight:700;margin-bottom:10px;font-size:1.1rem;')}">${escHtml(val)}</h3>`;
            if (seg.type === 'desc') return `<p data-sf-path="${path}" style="${BlockTypes.applySubStyle(props, path, 'color:inherit;opacity:.65;font-size:0.9rem;line-height:1.7;')}">${escHtml(val)}</p>`;
            return '';
        }).join('');
      ...
      const segsHtml = segments.map((s, i) => {
          ...
          if (s.type === 'badge') {
              const badgeText = resolveSubText(props, path, props.badge);
              return `<div data-sf-path="${path}" style="text-align:${props.badgeAlign || 'center'}; margin-bottom:16px;">
                <span style="background:${props.accentColor};color:#fff;padding:6px 16px;border-radius:99px;font-size:0.78rem;font-weight:600;display:inline-block;">${escHtml(badgeText)}</span>
              </div>`;
          }
          if (s.type === 'title') {
              const titleText = resolveSubText(props, path, props.title);
              return `<h2 data-sf-path="${path}" style="color:${props.textColor};font-size:clamp(1.8rem,4vw,2.6rem);font-weight:800;margin-bottom:16px;font-family:'Poppins',sans-serif; text-align:${props.titleAlign || 'center'};">${escHtml(titleText)}</h2>`;
          }
          if (s.type === 'subtitle') {
              const subtitleText = resolveSubText(props, path, props.subtitle);
              return `<p data-sf-path="${path}" style="color:${props.textColor};opacity:.65;max-width:650px;margin-bottom:48px;line-height:1.7; text-align:${props.subtitleAlign || 'center'}; margin-left:${props.subtitleAlign === 'center' ? 'auto' : (props.subtitleAlign === 'right' ? 'auto' : '0')}; margin-right:${props.subtitleAlign === 'center' ? 'auto' : (props.subtitleAlign === 'left' ? 'auto' : '0')};">${escHtml(subtitleText)}</p>`;
          }
          if (s.type === 'items') {
  ...
  carousel: {
    ...
    render(props) {
      ...
      const slidesHtml = (props.slides || []).map((slide, i) => {
        ...
        const slideObjectFit = slide.objectFit || objectFit;
        const slideObjectPosition = slide.objectPosition || 'center';
        const slideOpacity = slide.opacity !== undefined ? slide.opacity : 1;
        const slideFilters = [];
        if (slide.blur && parseFloat(slide.blur) > 0) slideFilters.push(`blur(${slide.blur}px)`);
        if (slide.brightness !== undefined && slide.brightness != 1) slideFilters.push(`brightness(${slide.brightness})`);
        if (slide.grayscale && parseFloat(slide.grayscale) > 0) slideFilters.push(`grayscale(${slide.grayscale})`);
        if (slide.saturate !== undefined && slide.saturate != 1) slideFilters.push(`saturate(${slide.saturate})`);
        const slideFilterCSS = slideFilters.length > 0 ? `filter:${slideFilters.join(' ')};` : '';
        const slideVisible = slide.visible === false ? 'display:none;' : '';
        ...
        const slideStyle = `width:100%;flex-shrink:0;position:relative;display:block;margin:0 !important;padding:0 !important;list-style:none !important;visibility:visible !important;min-height:${preserveFixedFrame ? props.height : '100px'};${preserveFixedFrame ? `height:${props.height};` : ''}${fitNeedsFrame ? 'overflow:hidden;' : ''}${slideVisible}`;
        const imageStyle = fitNeedsFrame
          ? `width:100%;height:${preserveFixedFrame || dynamicCoverHeight ? '100%' : 'auto'};display:block;border:none;visibility:visible;opacity:${slideOpacity};min-height:${preserveFixedFrame ? '100%' : '100px'};object-fit:${slideObjectFit};object-position:${slideObjectPosition};${slideFilterCSS}`
          : `width:100%;height:auto;display:block;border:none;visibility:visible;opacity:${slideOpacity};min-height:100px;object-fit:${slideObjectFit};object-position:${slideObjectPosition};${slideFilterCSS}`;
...
<\/script>`;
    ...
    render(props) {
      ...
      const header = `
        <div style="text-align:${headerAlign};margin-bottom:48px;">
          <span data-sf-path="segments.0.badge" style="${BlockTypes.applySubStyle(props, "segments.0.badge", `background:${props.accentColor};color:#fff;padding:4px 14px;border-radius:99px;font-size:0.78rem;font-weight:600;`)}">${resolveSubText(props, "segments.0.badge", props.badge)}</span>
          <h2 data-sf-path="segments.0.title" style="${BlockTypes.applySubStyle(props, "segments.0.title", `color:${props.textColor};font-size:clamp(1.6rem,3vw,2.4rem);font-weight:700;margin:16px 0 10px;font-family:'Poppins',sans-serif;`)}">${resolveSubText(props, "segments.0.title", props.title)}</h2>
          <p data-sf-path="segments.0.subtitle" style="${BlockTypes.applySubStyle(props, "segments.0.subtitle", `color:${props.textColor};opacity:.65;max-width:550px;line-height:1.7;margin:${headerAlign === 'center' ? 'auto' : '0'};`)}">${resolveSubText(props, "segments.0.subtitle", props.subtitle)}</p>
        </div>`;
      ...
      // --- New formFields-based rendering (if configured via panel) ---
      let formHtml;
      const inputStyle = `background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:13px 16px;color:${props.textColor};font-size:0.9rem;outline:none;width:100%;`;
      
      if (props.formFields && props.formFields.length > 0) {
          formHtml = props.formFields.map((ff, fi) => {
              const req = ff.required ? ' required' : '';
              const name = escAttr(ff.name || ff.type + fi);
              const ph = escAttr(ff.placeholder || ff.label || '');
              
              if (ff.type === 'textarea') {
                  return `<textarea name="${name}" placeholder="${ph}" rows="5"${req} style="${inputStyle}resize:vertical;"></textarea>`;
              }
              if (ff.type === 'select') {
                  const opts = (ff.options || ['Option 1', 'Option 2']).map(o => `<option value="${escAttr(o)}">${escHtml(o)}</option>`).join('');
                  return `<select name="${name}"${req} style="${inputStyle}cursor:pointer;"><option value="" disabled selected>${ph}</option>${opts}</select>`;
              }
              if (ff.type === 'checkbox') {
                  return `<label style="display:flex;align-items:center;gap:10px;color:${props.textColor};font-size:0.88rem;cursor:pointer;"><input type="checkbox" name="${name}"${req} style="width:18px;height:18px;cursor:pointer;" />${escHtml(ff.label || 'I agree')}</label>`;
              }
              if (ff.type === 'file') {
                  return `<input type="file" name="${name}"${req} style="${inputStyle}cursor:pointer;" />`;
              }
              // text, email, tel, number, url, date
              return `<input type="${escAttr(ff.type)}" name="${name}" placeholder="${ph}"${req} style="${inputStyle}" />`;
          }).join('');
          // Add submit button
          formHtml += `<button type="submit" style="background:${props.accentColor};color:#fff;border:none;border-radius:8px;padding:14px;font-size:0.95rem;font-weight:700;cursor:pointer;">Send Message ✉️</button>`;
      } else {
          // Legacy formSegments rendering
          formHtml = props.formSegments.map((s, i) => {
              if (s.type === 'row') {
                  const rowItems = (s.items || []).map((ri, rj) => {
                      if (ri.type === 'name') return `<input data-sf-path="formSegments.${i}.items.${rj}" type="text" placeholder="Your Name" required style="${inputStyle}" />`;
                      if (ri.type === 'email') return `<input data-sf-path="formSegments.${i}.items.${rj}" type="email" placeholder="Your Email" required style="${inputStyle}" />`;
                      return '';
                  }).join('');
              ...
              if (s.type === 'subject') return `<input data-sf-path="formSegments.${i}" type="text" placeholder="Subject" style="${inputStyle}" />`;
              if (s.type === 'message') return `<textarea data-sf-path="formSegments.${i}" placeholder="Your message…" rows="5" required style="${inputStyle}resize:vertical;"></textarea>`;
      ...
      // Form with backend integration support
      const formProvider = props.formProvider || 'none';
      const formConfig = props.formConfig || {};
      let formAttrs = 'onsubmit="event.preventDefault();alert(\'Message sent! (Demo)\');" style="display:flex;flex-direction:column;gap:14px;"';
      let formHiddenFields = '';
      if (typeof FormIntegrations !== 'undefined' && formProvider !== 'none') {
          formAttrs = FormIntegrations.generateFormTag(formProvider, formConfig) + ' style="display:flex;flex-direction:column;gap:14px;"';
          formHiddenFields = FormIntegrations.generateHiddenFields(formProvider, formConfig);
      }
      const formColumn = `<form ${formAttrs}>${formHiddenFields}${formHtml}</form>`;
  ...
  text: {
    ...
    render(props) {
      ...
      const subTextColor = props.textColor ? props.textColor + ';opacity:0.7' : '#475569';
    ...
    ${badge ? `<span data-sf-path="items.${i}.badge" style="display:inline-block;font-size:.78rem;letter-spacing:.16em;text-transform:uppercase;color:${accentColor};font-weight:700;margin-bottom:12px;">${badge}</span>` : ''}
    <h3 data-sf-path="items.${i}.title" style="font-size:${props.items.length === 1 ? '2.3rem' : '1.45rem'};font-weight:800;color:${textColor};margin:0 0 10px;line-height:1.15;">${title}</h3>
    <p data-sf-path="items.${i}.text" style="color:${textColor};opacity:0.7;line-height:1.75;margin:0;">${text}</p>
  </div>`;
        }).join('');
  ...
  #canvasFrame.mobile #${uid} .sf-text-inner {
    grid-template-columns: minmax(0, 1fr) !important;
        ...
        return `${responsiveStyles}<div id="${uid}" class="sf-text-block ${props.customClass || ''}" style="background:${props.bgColor}; color:${textColor}; ${BlockTypes.applyLayout(props)} text-align:${props.align};">
  <div class="sf-text-inner" style="max-width:1100px;margin:auto;display:grid;grid-template-columns:${gridTemplateColumns};gap:${props.gap || '18px'};align-items:start;">
    ${cards}
      ...
      if (props.title || props.text || props.badge) {
        return `<div id="${uid}" class="sf-text-block ${props.customClass || ''}" style="background:${props.bgColor}; color:${textColor}; ${BlockTypes.applyLayout(props)} text-align:${props.align};">
  <div style="max-width:1100px;margin:auto;">
    ${props.badge ? `<span data-sf-path="badge" style="display:inline-block;font-size:.8rem;letter-spacing:.18em;text-transform:uppercase;color:${accentColor};font-weight:700;margin-bottom:14px;">${props.badge}</span>` : ''}
    <h2 data-sf-path="title" style="font-size:2.3rem;font-weight:800;color:${textColor};margin:0 0 14px;line-height:1.1;">${props.title || 'Section Title'}</h2>
    <p data-sf-path="text" style="color:${textColor};opacity:0.7;line-height:1.8;max-width:760px;margin:${props.align === 'center' ? '0 auto' : '0'};">${props.text || ''}</p>
  </div>
</div>`;
      }
      return `<div id="${uid}" class="sf-text-block ${props.customClass || ''}" style="background:${props.bgColor}; color:${textColor}; ${BlockTypes.applyLayout(props)} text-align:${props.align};">
...
<\/script>`;
  ...
  motionPopup: {
    ...
    defaultProps: {
      ...
      popupBgColor: '#ffffff',
      popupBgImage: '',
      popupTextColor: '#0f172a',
      customId: '',
      customClass: ''
    },
    render(props) {
      const uid = props.customId || props._uid || 'motion_' + Math.random().toString(36).substr(2, 9);
...
<\/script>` : '';
      ...
      return `<section id="${uid}" class="sf-motion-popup ${props.customClass || ''}" style="background:${props.bgColor};padding:${props.padding};position:relative;overflow:visible;min-height:0;">
        <style>
          ...
          #${uid} .sf-motion-popup-card {
            width:min(92vw, ${props.popupWidth || '460px'}); background:${props.popupBgImage ? `linear-gradient(rgba(0,0,0,0.3),rgba(0,0,0,0.3)), url(${props.popupBgImage}) center/cover no-repeat` : (props.popupBgColor || '#ffffff')}; color:${props.popupTextColor || '#0f172a'}; border-radius:${props.popupRadius || '24px'}; padding:${props.popupPadding || '28px'};
        ...
        <div class="sf-motion-popup" onclick="if(event.target===this){this.classList.remove('is-open'); setTimeout(()=>this.style.display='none',260);}">
          <div class="sf-motion-popup-card" onclick="event.stopPropagation();" style="position:relative;">
            ...
            <div data-sf-path="popupTitle" style="${BlockTypes.applySubStyle(props, 'popupTitle', `font-size:1.5rem;font-weight:800;margin:0 0 12px;color:#0f172a;`)}">${resolveSubText(props, 'popupTitle', props.popupTitle || '')}</div>
            <div data-sf-path="popupText" style="${BlockTypes.applySubStyle(props, 'popupText', `font-size:0.96rem;line-height:1.75;color:#334155;margin-bottom:20px;`)}">${resolveSubText(props, 'popupText', props.popupText || '')}</div>
            <a data-sf-path="popupButtonText" href="${props.popupButtonHref || '#'}" style="${BlockTypes.applySubStyle(props, 'popupButtonText', `display:inline-flex;align-items:center;justify-content:center;padding:12px 18px;border-radius:999px;background:${props.accentColor};color:#fff;text-decoration:none;font-weight:700;`)}">${resolveSubText(props, 'popupButtonText', props.popupButtonText || 'Continue')}</a>
          </div>
        </div>
        ${popupScript}
      </section>`;
      ...
      const items = (props.items || []).map((item, i) => {
        ...
        return `
          <div data-sf-path="items.${i}" class="sf-accordion-item${isOpen ? ' is-open' : ''}" style="border-bottom:1px solid ${props.borderColor || 'rgba(255,255,255,0.16)'};">
            <button type="button" class="sf-accordion-trigger" data-acc-index="${i}" data-block-id="${uid}" onclick="var item=this.closest('.sf-accordion-item'); item.classList.toggle('is-open'); var panel=item.querySelector('.sf-accordion-panel'); if(panel) panel.style.display=item.classList.contains('is-open')?'block':'none'; this.setAttribute('aria-expanded', item.classList.contains('is-open') ? 'true' : 'false'); event.stopPropagation();" aria-expanded="${isOpen ? 'true' : 'false'}" style="width:100%;background:none;border:none;color:${props.textColor};padding:24px 16px;display:flex;align-items:center;justify-content:space-between;gap:16px;cursor:pointer;text-align:left;">
              <span data-sf-path="items.${i}.title" style="${BlockTypes.applySubStyle(props, `items.${i}.title`, `font-size:clamp(1.05rem,2vw,1.24rem);font-weight:700;letter-spacing:0.01em;color:${props.textColor};text-transform:uppercase;`)}">${resolveSubText(props, `items.${i}.title`, item.title || 'Item ' + (i + 1))}</span>
              <span class="sf-accordion-icon" style="width:34px;height:34px;display:inline-flex;align-items:center;justify-content:center;color:${props.accentColor || props.textColor};font-size:1rem;transition:transform .25s ease;flex-shrink:0;"><i class="fa-solid fa-chevron-down"></i></span>
            </button>
            <div class="sf-accordion-panel" style="display:${isOpen ? 'block' : 'none'};padding:0 0 28px;">
              <div class="sf-accordion-panel-inner" data-sf-path="items.${i}.contentHtml" style="background:${props.cardBg || '#0a0a0a'};border-radius:18px;padding:${item.image || item.heading || item.description ? '20px' : '0'};color:${props.textColor};overflow:auto;">
                ${item.image || item.heading || item.description ? `
                  <div style="display:grid;grid-template-columns:${item.image ? '1fr 1.2fr' : '1fr'};gap:18px;align-items:start;">
                    ${item.image ? `<img src="${item.image}" alt="${item.title || ''}" style="width:100%;border-radius:14px;display:block;" />` : ''}
                    <div>
                      ${item.heading ? `<div style="font-size:1.5rem;font-weight:800;color:${props.accentColor || '#f4b400'};margin-bottom:10px;">${item.heading}</div>` : ''}
                      ${item.description ? `<p style="color:${props.textColor};line-height:1.8;opacity:0.85;">${item.description}</p>` : ''}
                    </div>
                  </div>
                ` : (item.contentHtml || '')}
              </div>
  ...
  audioPlayer: {
    ...
    render(props) {
      const uid = props.customId || 'audio_' + Math.random().toString(36).slice(2, 9);
      const resolvedSrc = (typeof AssetManager !== 'undefined' && AssetManager.resolve) ? AssetManager.resolve(props.src) : props.src;
      return `<div id="${uid}" class="sf-audio-player ${props.customClass || ''}" style="background:${props.bgColor};color:${props.textColor};padding:${props.padding};border-radius:${props.borderRadius};${BlockTypes.applyLayout(props)}">
  <div style="display:flex;align-items:center;gap:18px;max-width:800px;margin:auto;">
    <div style="width:64px;height:64px;border-radius:14px;background:${props.accentColor};display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 8px 24px rgba(0,0,0,0.3);">
      <i class="fa-solid fa-music" style="font-size:1.4rem;color:#fff;"></i>
    </div>
    <div style="flex:1;min-width:0;">
      <div data-sf-path="title" style="font-size:1.1rem;font-weight:700;margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${props.title}</div>
      <div data-sf-path="artist" style="font-size:0.85rem;opacity:0.6;margin-bottom:12px;">${props.artist}</div>
      <audio controls style="width:100%;height:36px;border-radius:8px;outline:none;" ${props.autoplay ? 'autoplay' : ''} ${props.loop ? 'loop' : ''}>
  ...
  mapEmbed: {
    ...
    render(props) {
      const uid = props.customId || 'map_' + Math.random().toString(36).slice(2, 9);
      const titleHtml = props.showTitle && props.title
        ? `<h3 data-sf-path="title" style="font-size:1.4rem;font-weight:700;color:${props.textColor};margin:0 0 16px;text-align:center;">${props.title}</h3>`
        : '';
      return `<div id="${uid}" class="sf-map-embed ${props.customClass || ''}" style="background:${props.bgColor};${BlockTypes.applyLayout(props)}${props.padding ? 'padding:'+props.padding+';' : ''}">
  ${titleHtml}
  <div style="width:100%;height:${props.height};border-radius:${props.borderRadius};overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
    <iframe src="${props.embedUrl}" width="100%" height="100%" style="border:0;display:block;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
  </div>
</div>`;
    }
  },
  ...
  backToTop: {
    ...
    render(props) {
      const uid = props.customId || 'btt_' + Math.random().toString(36).slice(2, 9);
      const posMap = {
        'bottom-right': `right:${props.offset};`,
        'bottom-left': `left:${props.offset};`,
        'bottom-center': `left:50%;transform:translateX(-50%);`
      };
      const posStyle = posMap[props.position] || posMap['bottom-right'];
      return `<div id="${uid}" class="sf-back-to-top ${props.customClass || ''}" style="position:relative;width:100%;min-height:120px;display:flex;align-items:center;justify-content:${props.position === 'bottom-left' ? 'flex-start' : props.position === 'bottom-center' ? 'center' : 'flex-end'};padding:32px ${props.offset};background:transparent;border:none;">
  <button type="button" onclick="window.scrollTo({top:0,behavior:'${props.smooth ? 'smooth' : 'auto'}'})" style="width:${props.size};height:${props.size};border-radius:${props.borderRadius};background:${props.accentColor};color:${props.textColor};border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:1.3rem;box-shadow:0 8px 24px rgba(0,0,0,0.2);transition:transform 0.2s,box-shadow 0.2s;" onmouseover="this.style.transform='scale(1.1)';this.style.boxShadow='0 12px 32px rgba(0,0,0,0.3)'" onmouseout="this.style.transform='';this.style.boxShadow='0 8px 24px rgba(0,0,0,0.2)'">
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 19V5"/><path d="M5 12l7-7 7 7"/></svg>
  </button>
</div>
<style>
  /* In exported/preview site: fixed position, no space */
  body > .sf-back-to-top#${uid},
  body > div > .sf-back-to-top#${uid} {
  ...
  body > .sf-back-to-top#${uid}.sf-btt-visible,
  body > div > .sf-back-to-top#${uid}.sf-btt-visible {
    opacity:1;
    pointer-events:auto;
  }
</style>
...
(function(){
  ...
  if (!el) return;
  // Only activate fixed behavior outside the editor canvas
  if (el.closest('#canvas') || el.closest('.block-content')) return;
  var threshold = ${props.showAfter || 300};
  function check() {
    var scrollY = window.pageYOffset || document.documentElement.scrollTop;
    if (scrollY > threshold) {
      el.classList.add('sf-btt-visible');
    } else {
      el.classList.remove('sf-btt-visible');
    }
  }
  ...
  applyLayout(props) {
    ...
    const alignSelf = props.alignSelf && props.alignSelf !== 'auto' ? `align-self:${props.alignSelf};` : '';
    const minWidth = props.minWidth ? `min-width:${props.minWidth};` : '';
    const maxWidth2 = props.maxWidth ? `max-width:${props.maxWidth};` : '';
    const minHeight = props.minHeight ? `min-height:${props.minHeight};` : '';
    const maxHeight = props.maxHeight ? `max-height:${props.maxHeight};` : '';
    
    // Grid properties
    const gridCols = props.gridTemplateColumns ? `grid-template-columns:${props.gridTemplateColumns};` : '';
    const gridRows = props.gridTemplateRows ? `grid-template-rows:${props.gridTemplateRows};` : '';
    const colGap = props.columnGap ? `column-gap:${props.columnGap};` : '';
    const rowGap = props.rowGap ? `row-gap:${props.rowGap};` : '';
    const justifyItems = props.justifyItems ? `justify-items:${props.justifyItems};` : '';
    const placeContent = props.placeContent ? `place-content:${props.placeContent};` : '';
    ...
    let layoutCore = `width:${props.width || '100%'}; ${heightStyle} margin:${props.margin || '0'}; padding:${props.padding || '0'}; display:${display}; box-sizing:border-box;`;
    
    if (isFlex) {
      layoutCore += ` flex-direction:${props.direction || 'row'}; justify-content:${props.justify || 'center'}; align-items:${props.align || 'center'}; gap:${props.gap || '0'}; flex-wrap:${props.wrap || 'wrap'};`;
    } else if (isGrid) {
      layoutCore += ` ${gridCols} ${gridRows} ${colGap} ${rowGap} ${justifyItems} ${placeContent} gap:${props.gap || '0'};`;
    }
    
    // Background image/gradient (applied via applyLayout so ALL blocks support it)
    // Uses the full `background` shorthand to override any earlier `background:` declarations
    let bgStyle = '';
    if (props.bgImage) {
        // Resolve asset path to data URL for canvas rendering
        const resolvedBgImage = (typeof AssetManager !== 'undefined' && AssetManager.resolve) 
            ? AssetManager.resolve(props.bgImage) 
            : props.bgImage;
        const overlayOpacity = props.bgOverlayOpacity !== undefined ? props.bgOverlayOpacity : 0.4;
        const overlayColor = props.bgOverlayColor || '0,0,0';
        const bgSize = props.bgSize || 'cover';
        const bgPosition = props.bgPosition || 'center';
        const hasBlur = props.bgBlur && parseInt(props.bgBlur) > 0;
        
        if (hasBlur) {
            // When blur is needed, don't set background on the element itself
            // The render function will add a <style> with ::before for the blurred bg
            bgStyle = `position:relative; overflow:hidden;`;
        } else {
            bgStyle = `background: linear-gradient(rgba(${overlayColor},${overlayOpacity}), rgba(${overlayColor},${overlayOpacity})), url(${resolvedBgImage}) ${bgPosition}/${bgSize} no-repeat !important;`;
        }
    } else if (props.background && props.background !== 'transparent' && props.background !== 'none') {
        bgStyle = `background:${props.background} !important;`;
    }

    // Border (solid or gradient)
    let borderStyle = '';
    const borderMode = props.borderMode || 'always';
    const hasGradientBorder = props.borderGradient && props.borderGradient.trim();
    if ((props.borderStyle === 'gradient' || hasGradientBorder) && props.borderGradient) {
        const bw = props.borderWidth || '2px';
        const br = props.borderRadius || '0';
        const grad = props.borderGradient;
        if (borderMode === 'always') {
            borderStyle = `border:${bw} solid transparent;background-image:linear-gradient(var(--sf-bg,#0f172a),var(--sf-bg,#0f172a)),${grad};background-origin:border-box;background-clip:padding-box,border-box;border-radius:${br};`;
        } else {
            borderStyle = `border:${bw} solid transparent;border-radius:${br};`;
        }
    } else if (props.borderStyle && props.borderStyle !== 'none' && props.borderStyle !== 'gradient') {
        if (borderMode === 'always') {
            borderStyle = `border:${props.borderWidth || '1px'} ${props.borderStyle} ${props.borderColor || '#333'};`;
        } else {
            borderStyle = `border:${props.borderWidth || '1px'} ${props.borderStyle} transparent;`;
        }
    }
    if (props.borderRadius && !borderStyle.includes('border-radius')) {
        borderStyle += `border-radius:${props.borderRadius};`;
    }

    // Border animation
    let borderAnim = '';
    if (props.borderAnimation && props.borderAnimation !== 'none') {
        const speed = props.borderAnimSpeed || '3';
        if (props.borderAnimation === 'rainbow') {
            borderAnim = `animation:sf-border-rainbow ${speed}s linear infinite;background-size:200% 200%;`;
        } else if (props.borderAnimation === 'spin') {
            borderAnim = `animation:sf-border-spin ${speed}s linear infinite;`;
        } else if (props.borderAnimation === 'pulse') {
            borderAnim = `animation:sf-border-pulse ${speed}s ease-in-out infinite;`;
        }
    }

    // Hover effects (including border-on-hover mode)
    let hoverStyle = '';
    if (props.hoverScale || props.hoverShadow || props.hoverBorderColor || borderMode === 'hover') {
        hoverStyle = 'transition:transform 0.3s,box-shadow 0.3s,border-color 0.3s,background-image 0.3s,opacity 0.3s;cursor:pointer;';
    }

    // Background animation
    let bgAnimStyle = '';
    if (props.bgAnimation && props.bgAnimation !== 'none') {
        const bgSpeed = props.bgAnimSpeed || '5';
        if (props.bgAnimation === 'gradient-shift') {
            bgAnimStyle = `background-size:200% 200%;animation:sf-bg-gradient-shift ${bgSpeed}s ease infinite;`;
        } else if (props.bgAnimation === 'gradient-rotate') {
            bgAnimStyle = `animation:sf-bg-gradient-rotate ${bgSpeed}s linear infinite;`;
        } else if (props.bgAnimation === 'color-pulse') {
            bgAnimStyle = `animation:sf-bg-color-pulse ${bgSpeed}s ease-in-out infinite;`;
        } else if (props.bgAnimation === 'hue-rotate') {
            bgAnimStyle = `animation:sf-bg-hue-rotate ${bgSpeed}s linear infinite;`;
        }
    }
    
    return `${layoutCore} ${minWidth} ${maxWidth2} ${minHeight} ${maxHeight} ${bgStyle} ${bgAnimStyle} ${borderStyle} ${borderAnim} ${hoverStyle} ${opacity} ${blur} ${grow} ${shrink} ${basis} ${overflow} ${alignSelf} ${animation}`;
  },

  /**
   * Returns onmouseover/onmouseout HTML attributes for hover effects.
   * Use in render: <div style="..." ${BlockTypes.applyHover(props)}>
   */
  applyHover(props) {
    if (!props) return '';
    const borderMode = props.borderMode || 'always';
    const hoverIn = [];
    const hoverOut = [];
    if (props.hoverScale) { hoverIn.push(`this.style.transform='scale(${props.hoverScale})'`); hoverOut.push(`this.style.transform=''`); }
    if (props.hoverShadow) { hoverIn.push(`this.style.boxShadow='${props.hoverShadow}'`); hoverOut.push(`this.style.boxShadow='${props.boxShadow || 'none'}'`); }
    if (props.hoverBorderColor) { hoverIn.push(`this.style.borderColor='${props.hoverBorderColor}'`); hoverOut.push(`this.style.borderColor='${borderMode === 'hover' ? 'transparent' : (props.borderColor || 'transparent')}'`); }
    if (borderMode === 'hover' && props.borderGradient) {
        const grad = props.borderGradient;
        hoverIn.push(`this.style.backgroundImage='linear-gradient(var(--sf-bg,#0f172a),var(--sf-bg,#0f172a)),${grad.replace(/'/g,"\\'")}'`);
        hoverIn.push(`this.style.backgroundOrigin='border-box'`);
        hoverIn.push(`this.style.backgroundClip='padding-box,border-box'`);
        hoverOut.push(`this.style.backgroundImage=''`);
        hoverOut.push(`this.style.backgroundOrigin=''`);
        hoverOut.push(`this.style.backgroundClip=''`);
    } else if (borderMode === 'hover' && props.borderColor) {
        hoverIn.push(`this.style.borderColor='${props.borderColor}'`);
        hoverOut.push(`this.style.borderColor='transparent'`);
    }
    if (hoverIn.length === 0) return '';
    return `onmouseover="${hoverIn.join(';')}" onmouseout="${hoverOut.join(';')}"`;
  },
  ...
  applySubStyle(props, path, baseStyle = '') {
    ...
    if (s.background) rules.push(`background:${s.background}`);
    ...
    if (s.minWidth) rules.push(`min-width:${s.minWidth}`);
    ...
    if (s.maxHeight) rules.push(`max-height:${s.maxHeight}`);
    ...
    if (s.overflow) rules.push(`overflow:${s.overflow}`);
    // Flex/Grid child properties
    if (s.alignSelf) rules.push(`align-self:${s.alignSelf}`);
    if (s.flexGrow !== undefined) rules.push(`flex-grow:${s.flexGrow}`);
    if (s.flexShrink !== undefined) rules.push(`flex-shrink:${s.flexShrink}`);
    if (s.flexBasis) rules.push(`flex-basis:${s.flexBasis}`);
    // Flex/Grid parent properties
    ...
    if (s.wrap) rules.push(`flex-wrap:${s.wrap}`);
    // Grid-specific
    if (s.gridTemplateColumns) rules.push(`grid-template-columns:${s.gridTemplateColumns}`);
    if (s.gridTemplateRows) rules.push(`grid-template-rows:${s.gridTemplateRows}`);
    if (s.columnGap) rules.push(`column-gap:${s.columnGap}`);
    if (s.rowGap) rules.push(`row-gap:${s.rowGap}`);
    if (s.justifyItems) rules.push(`justify-items:${s.justifyItems}`);
    if (s.placeContent) rules.push(`place-content:${s.placeContent}`);
